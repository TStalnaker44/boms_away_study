...
Important note: `short-answer` should be used for questions where users are unlikely to provide duplicate answers to questions, whereas `single-text` should be used where duplicated answers are possible (e.g. "How old are you?). `short-anwer` questions will not be included in figures generated (unless response coding, see below, has been completed), as it is expected that such answers would all be unique.
